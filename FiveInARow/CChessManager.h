...
public:
    CChessManager();
    ~CChessManager();
    void NewGame() { m_nChess = 0; m_Color = BLACK; } // 开始新的一局
    bool Xy2Xy(int x0, int y0, int &x1, int &y1); // 物理坐标转成逻辑坐标, 成功返回true
    int Add(int x, int y); // 在物理坐标 (x, y) 处落子, 成功返回0, 没点中返回1, 重复返回2
