...
BEGIN_MESSAGE_MAP(CFiveInARowDlg, CDialogEx)
	...
	ON_WM_LBUTTONUP()
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_BUTTON_MUSIC, &CFiveInARowDlg::OnBnClickedButtonMusic)
END_MESSAGE_MAP()
...
BOOL CFiveInARowDlg::OnInitDialog()
{
	...
	PlaySound(_T("res\\music.wav"), NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);	//播放背景音乐
...
/*
void CFiveInARowDlg::OnBnClickedButtonMusic()
{
	//TODO: 在此添加控件通知处理程序代码
	if (m_musicState == true)	//状态：播放；执行；停止播放
	{
		PlaySound(NULL, NULL, 0);	//停止播放
		m_musicState = false;		//状态：停止
	}
	else						//状态：停止；执行；播放
	{
		PlaySound(_T("res\\music.wav"), NULL, SND_FILENAME | SND_ASYNC);	//播放背景音乐
		m_musicState = true;			//状态：播放
	}
}
*/
int State = 0;	//变量，记录音乐状态，0为停止，1为播放
void CFiveInARowDlg::OnBnClickedButtonMusic()
{
	// TODO: 在此添加控件通知处理程序代码
	if (State == 0)	//状态：停止；执行；播放
	{
		PlaySound(_T("res\\music.wav"), NULL, SND_FILENAME | SND_ASYNC );	//播放背景音乐
		State = 1;	//状态：播放
	}
	...
	{
		PlaySound(NULL, NULL, SND_FILENAME | SND_ASYNC);	//停止播放
		State = 0;	//状态：停止
	}
}
